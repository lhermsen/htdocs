...
// chart, chartData
  ...
  var pullOutBorderStyle = "#FFFFFF";                  // Colour of the pull-out slice border
  ...
  function init() {
    ...
    canvas = document.getElementById(chartNaam);
    ...
    $('#' + chartNaam + 'Data td').each( function() {
      currentCell++;
      if ( currentCell % 2 != 0 ) {
        currentRow++;
    ...
    drawChart();
    $('#' + chartNaam + '').click ( handleChartClick );
  }
  ...
  function startPullOut ( slice ) {
    ...
    $('#' + chartNaam + 'Data td').removeClass('highlight');
    var labelCell = $('#' + chartNaam + 'Data td:eq(' + (slice*2) + ')');
    var valueCell = $('#' + chartNaam + 'Data td:eq(' + (slice*2+1) + ')');
  ...
  function drawSlice ( context, slice ) {
    ...
    if ( slice == currentPullOutSlice ) {
      ...
      context.fillText( pullOutValuePrefix + chartData[slice]['value'] + " leden (" + ( parseInt( chartData[slice]['value'] / totalValue * 100 + .5 ) ) +  "%)", centreX + Math.cos(midAngle) * ( chartRadius + maxPullOutDistance + pullOutLabelPadding ), centreY + Math.sin(midAngle) * ( chartRadius + maxPullOutDistance + pullOutLabelPadding ) + 15 );
